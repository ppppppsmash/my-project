...
>>>>>>> develop/liming.pei
      ...
      const options = {
        //hostname: process.env.API_BASE_URL_SCHEDULER,
        hostname: '127.0.0.1',
        port: 3000,
        path: `/psi?${queryParams}`,
        method: 'GET'
      }
      ...
      const req = http.request(options, (res) => {
        ...
        res.on('end', async () => {
          const { lighthouseResult, loadingExperience } = JSON.parse(data)
      ...
      req.on('error', (error) => {
        console.error(`HTTP Request Error for ${name} ${id}:`, error)
      })
  ...
  addCronJobHours(job_name: string, time: string, name: string, url: string, device: string, id: number) {
      //const job = new CronJob(`${time} * * * * *`, async () => { //テスト用
